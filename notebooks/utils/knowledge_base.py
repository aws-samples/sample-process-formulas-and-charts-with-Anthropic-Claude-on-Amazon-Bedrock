...
class BedrockKnowledgeBase:
    ...
    def __init__(
        ...
        self.kb_execution_role_name = f'BedrockExecRoleKB_{self.suffix}'
        self.fm_policy_name = f'BedrockFoundationModelPolicyKB_{self.suffix}'
        self.s3_policy_name = f'BedrockS3PolicyKB_{self.suffix}'
        self.oss_policy_name = f'BedrockOSSPolicyKB_{self.suffix}'
        self.roles = []
    ...
    def create_knowledge_base(self):
        ...
        while kb_status != 'ACTIVE':
            print(f"Waiting for knowledge base to be active. Current status: {kb_status}")
            interactive_sleep(10)  # Sleep for 10 seconds between checks
            response = self.bedrock_agent_client.get_knowledge_base(knowledgeBaseId=kb['knowledgeBaseId'])
            kb = response['knowledgeBase']
            kb_status = kb['status']
        
        print("Knowledge base is now ACTIVE")            
